...
% 读取相对路径下的数据文件
filepath = '../result/data/v132_model_evaluate.mat';
...
num_list = reward_data.v132_episodes_list;    % 迭代周期
...
[min_total,min_total_index] = min(0.99*reward_value(1,:)/max(reward_value(1,:))+0.01*reward_value(2,:)/max(reward_value(2,:)));
disp(['目标函数最小值为:',num2str(min_total)])
disp(['目标函数最小值的位置:',num2str(num_list(min_total_index))])
disp(['该位置的时延为:',num2str(reward_value(1,min_total_index))])
disp(['该位置的能耗为:',num2str(reward_value(2,min_total_index))])
