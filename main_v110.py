...
if __name__ == '__main__':
    ...
    setting.reward_type = 10                             # 奖励类型
    setting.eps = 0.1                                   # 探索率
    setting.beta = setting.beta                         # 时延占比
    ...
    residual_packets = setting.MAX_TASK  # 初始化剩余数据包
    total_step = 0
    ...
    for i_episode in range(num_episodes):
        ...
        for e_i in range(episode_length):
            ...
            if done:
                ...
                break
            elif e_i == episode_length-1:   # 提前到达终点,这里是否会有问题？之前的实验目标导向没生效
                # rews += setting.terminal_reward/setting.MAX_TASK*(setting.MAX_TASK-
                #         sum(env.node_package_num_dict.values()))   # 目标导向
                residual_packets = sum(env.node_package_num_dict.values())
                rews = [rew+setting.unit_reward**(setting.MAX_TASK-residual_packets) for rew in rews]
                replay_buffer.add(obs, acts, rews, next_obs, done, precursors)  # 这个属于异常样本，是否会影响实验结果？
            else:
                replay_buffer.add(obs, acts, rews, next_obs, done, precursors)
        ...
        return_list.append(ep_returns)
        logger.info(f"Episode: {i_episode + 1},{ep_returns}，residual_packets:{residual_packets}")
