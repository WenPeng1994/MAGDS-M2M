...
class Topo:
    def __init__(self,xml_path='./topo.xml',traffic_matrix=setting.traffic_matrix_1):
        ...
        self.G,self.src_node_set,self.std_node_set,self.mid_node_set,self.node_power,\
            self.node_dicts = self._parse_xml()
        self.data_init()
    ...
    def _parse_xml(self):
        ...
        for child in links_element.iter():  # 遍历边元素
            if child.tag == 'link':
                ...
                G.add_edge(src,std)
        return G,src_node_set,std_node_set,mid_node_set,node_power,node_dicts
...
def run(topo):
    ...
    while True:
            ...
            if topo.node_dicts[current_hop].buffersize() > 0:  # 判断节点中是否有数据，有数据才进行处理
                for _ in range(topo.node_power[current_hop]):  # 根据处理能力进行传递
                    try:  # 防止出现空缓冲区pop的错误
                        ...
                        link_msg_dict[(current_hop, next_hop)][(src,std)] += 1
                        link_msg_dict[(current_hop,next_hop)].setdefault('total',0)
                        link_msg_dict[(current_hop,next_hop)]['total'] += 1
                    except:
                        continue
        ...
        # 记录所有节点上的信息量变化情况
        for current_hop in topo.G.nodes:
            msg_num_dict[current_hop].append(topo.node_dicts[current_hop].size())  # 终止条件

        if all([node_size == 0 for node_size in node_size_list]):
            break
        else:
            count += 1
    return msg_num_dict,count,link_msg_dict


# 性能展示
def performance_display(topo,msg_num_dict,count,link_msg_dict):
    # 统计各节点数据量的变化
    # 绘制各个节点上数据量变化曲线
...
if __name__ == '__main__':
    ...
    # print(topo.G.nodes)
    msg_num_dict, count, link_msg_dict = run(topo)
    # print(msg_num_dict,count)
    # print(link_msg_dict)
    performance_display(topo,msg_num_dict,count,link_msg_dict)
    ...
    3. 实现剩余能量统计 
    能量使用统计，每个节点发送一个数据包能量为105mW，接收一个数据包能量为54mW 20240304
    4. 实现链路利用率的统计
    """
