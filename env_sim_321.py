...
import pickle
from comments.utils import random_simulation
...
def run(topo,use_algorithms):
    ...
    while True:
        # 各节点处理信息传出
        for current_hop in topo.G.nodes:
            node_energy_use_dict.setdefault(current_hop, 0)  # 保证每个节点上都有值
            if topo.node_dicts[current_hop].buffersize() > 0:  # 判断节点中是否有数据，有数据才进行处理
                for _ in range(topo.node_power[current_hop]):  # 根据处理能力进行传递
                    try:  # 防止出现空缓冲区pop的错误
                        ...
                        next_hop = use_algorithms(topo.G,current_hop,std)
                        # 加入重传机制,加入时间限制
...
if __name__ == '__main__':
    ...
    # print(topo.G.nodes)
    msg_num_dict, count, link_msg_dict,node_energy_use_dict = run(topo,use_algorithms)
    ...
    # 构造一个用于测试的数据
    sample = 20
    node_package_num_dict = {}
    for node in topo.G.nodes:
        node_package_num_dict[node] = msg_num_dict[node][sample-1]

    print(node_package_num_dict)

    attributes_peer = {'package_num': node_package_num_dict, 'energy_use': node_energy_use_dict}
    with open('attribute_dict.pkl','wb') as file:
        pickle.dump(attributes_peer,file)
    new_G = add_node_attributes(topo.G,attributes_peer)

    # 保存图数据用于测试
    nx.write_gpickle(topo.G,'graph.pkl')

    pyg_data = from_networkx(new_G)
    print(pyg_data)
    torch.save(pyg_data,'gdata.pt')
    print('调试')

    # 数据归一化，使得每一列的和为1
    # x = pyg_data.x / pyg_data.x.sum(0,keepdims=True)
    # print(x)
